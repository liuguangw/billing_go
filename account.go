...
func getAccountByUsername(db *sql.DB,username string)(*Account,byte){
	...
	defer rows.Close()
	if rows.Next() {
		err = rows.Scan(&account.id,&account.name,&account.password,&account.question,&account.answer,&account.email,&account.qq,&account.point,&account.is_online,&account.is_lock)
		if err != nil {
			return &account,2
		}
	}else{
